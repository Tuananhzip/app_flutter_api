...
class DatabaseHelper {
  ...
  Future<void> insertProduct(Cart productModel) async {
    ...
    final existingCart = await _getCart(productModel.productID);
    if (existingCart != null) {
      productModel.count = existingCart.count + 1;
      await db.update(
        'Cart',
        productModel.toMap(),
        where: 'productID = ?',
  ...
  Future<Cart?> _getCart(int id) async {
    final db = await _databaseService.database;
  ...
  Future<int> getQuantityCart() async {
    final db = await _databaseService.database;
    final List<Map<String, dynamic>> maps = await db.query('Cart');
    return maps.length;
  }
